...
from app.models import Chamado, LogChamado, StatusChamado, User, Role, VisualizacaoChamado
...
def criar_chamado(db: Session, chamado_data: dict, usuario_id: int) -> Chamado:
    ...
    # Notifica os administradores da organizacao sobre o novo chamado
    ...
    for admin in admins:
        criar_notificacao(
            db,
            organizacao_id=admin.organizacao_id,
            usuario_id=admin.id,
            titulo="Novo Chamado Criado",
            mensagem=f"O chamado #{chamado.id} ({chamado.titulo}) foi aberto por {user.nome}.",
...
def atribuir_tecnico(
    ...
    criar_notificacao(
        ...
        mensagem=f"O chamado #{chamado.id} foi atribuído a você.", 
        chamado_id=chamado.id
    )
...
def finalizar_chamado(
    ...
    # Notifica o usuario dono do chamado
    criar_notificacao(
        db, 
        organizacao_id=chamado.organizacao_id, 
        usuario_id=chamado.usuario_id, 
        titulo="Chamado Finalizado", 
        mensagem=f"Seu chamado #{chamado.id} foi finalizado pelo técnico.", 
        chamado_id=chamado.id
    )
...
def adicionar_log(
    ...
    if autor_id == chamado.usuario_id and chamado.tecnico_id:
        # Usuário enviou msg -> notifica o técnico
        criar_notificacao(
            db, 
            organizacao_id=chamado.organizacao_id, 
            usuario_id=chamado.tecnico_id, 
            titulo="Nova Mensagem no Chamado", 
            mensagem=f"Usuário enviou mensagem no chamado #{chamado.id}", 
            chamado_id=chamado.id
        )
    elif autor_id == chamado.tecnico_id:
        # Técnico enviou msg -> notifica o usuário
        criar_notificacao(
            db, 
            organizacao_id=chamado.organizacao_id, 
            usuario_id=chamado.usuario_id, 
            titulo="Nova Mensagem no Chamado", 
            mensagem=f"Técnico enviou mensagem no chamado #{chamado.id}", 
            chamado_id=chamado.id
        )
...
def registrar_visualizacao_chamado(db: Session, chamado_id: int, usuario_id: int):
    """Registra que o usuário visualizou o chamado, se ainda não houver registro para hoje ou nos últimos mins."""
    # Para simplificar e evitar spam de visualizações no mesmo momento, vamos apenas salvar todas as aberturas
    # ou filtrar por tempo se necessitar (ex: apenas uma view por sessao).
    chamado = db.query(Chamado).filter(Chamado.id == chamado_id).first()
    if not chamado:
        return None
    user = db.query(User).filter(User.id == usuario_id).first()
    ...
    # Evita dupla contagem se já visualizou ha menos de 5 min (opcional, aqui salvaremos a cada GET para historico)
    view = VisualizacaoChamado(
        chamado_id=chamado_id,
        usuario_id=usuario_id,
        visualizado_em=datetime.utcnow()
    )
    db.add(view)
    db.commit()
    return view
