...
export default function ChamadosPage() {
    ...
    const [form, setForm] = useState({ titulo: '', descricao: '', prioridade: 'MEDIA', maquina_id: '' });
    ...
    const detalheModalRef = useRef(detalheModal);
    detalheModalRef.current = detalheModal;

    useEffect(() => { carregar(); }, [filtro]);

    // === TEMPO REAL via WebSocket ===
    // Quando chamado é criado ou atualizado, recarrega a lista
    useWebSocketEvent('CHAMADO_CRIADO', () => {
        console.log('[RT] Chamado criado detectado, recarregando...');
        carregar();
    });

    useWebSocketEvent('CHAMADO_ATUALIZADO', () => {
        console.log('[RT] Chamado atualizado detectado, recarregando...');
        carregar();
    });

    // Quando chega nova mensagem no chat do chamado aberto, recarrega mensagens
    useWebSocketEvent('NOVA_MENSAGEM', (data) => {
        const modal = detalheModalRef.current;
        if (modal && data.chamado_id === modal.id) {
            console.log('[RT] Nova mensagem no chamado aberto, recarregando chat...');
            listarMensagens(modal.id).then(setMensagens).catch(() => {});
        }
    });
    ...
    useEffect(() => {
        ...
        // Carrega maquinas para associar ao chamado
    ...
    const handleCriar = async (e) => {
        ...
        try {
            ...
            setModal(null);
            setForm({ titulo: '', descricao: '', prioridade: 'MEDIA', maquina_id: '' });
            setImagemFile(null);
            carregar();
        } catch (err) {
                    ...
                    <tbody>
                        {chamados.map(c => (
                            <tr key={c.id}>
                                ...
                                <td><span className={`badge badge-${c.prioridade.toLowerCase()}`}>{c.prioridade}</span></td>
                ...
                <div className="modal-overlay" onClick={() => setDetalheModal(null)}>
                    <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '650px' }}>
                        ...
                        <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                            ...
                            <span className={`badge badge-${detalheModal.prioridade.toLowerCase()}`}>{detalheModal.prioridade}</span>
                        </div>
                        ...
                        {role === 'ADMIN' && detalheModal.visualizacoes && detalheModal.visualizacoes.length > 0 && (
                            <div style={{ background: 'rgba(255,255,255,0.02)', borderRadius: 10, padding: 16, marginBottom: 16, border: '1px solid rgba(255,255,255,0.1)' }}>
                                <h4 style={{ color: '#fff', margin: '0 0 10px', fontSize: 14 }}>👁️ Histórico de Visualizações</h4>
                                <div style={{ maxHeight: 150, overflowY: 'auto' }}>
                                    {detalheModal.visualizacoes.map((v, idx) => (
                                        <div key={idx} style={{ padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: 13 }}>
                                            <span style={{ color: '#4FC3F7', fontWeight: 600 }}>{v.usuario?.nome || 'Usuario'}</span> visualizou em <span style={{ color: '#a0a0b0' }}>{new Date(v.visualizado_em).toLocaleString('pt-BR')}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
