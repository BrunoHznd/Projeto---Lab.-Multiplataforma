...
export default function NotificationBell({ user }) {
    ...
    useWebSocketEvent('NOTIFICACAO', (data) => {
        ...
        // Ativa notificacao nativa do Desktop (Electron)
    ...
    return (
        <div style={{ position: 'relative', cursor: 'pointer', marginRight: '10px' }} onClick={() => setAberto(!aberto)}>
            <div style={{ padding: '4px', fontSize: '18px', position: 'relative' }}>
                🔔
            ...
            {aberto && (
                <div style={{
                    ...
                    {notificacoes.length === 0 ? (
                        <p style={{ color: 'var(--cor-texto-sec)', fontSize: '12px', textAlign: 'center' }}>Nenhuma notificação</p>
                    ) : (
                        notificacoes.map(n => (
                            <div key={n.id} style={{
