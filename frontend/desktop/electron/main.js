...
if (process.platform === 'win32') {
    app.setAppUserModelId('tiresolve.desktop');
}
...
app.whenReady().then(() => {
    ...
    ipcMain.on('notify-ticket-created', (_event, payload) => {
        console.log('[IPC] notify-ticket-created', payload);
        try {
            if (!Notification.isSupported()) return;
            const titulo = payload?.titulo || 'Novo ticket';
            const solicitante = payload?.solicitante || 'N/A';
            ...
            // evita quebrar o processo principal por falha de notificação
            console.error(err);
        }
    });
